...
class EventJob < ApplicationJob
  ...
  def handle_stripe_event(event)
    ...
    when "checkout.session.completed"
      ...
      raise "Reservation not found for checkout session ID: #{checkout_session.id}" if reservation.nil?
    ...
    when "charge.refunded"
      ...
      raise "Reservation not found for Payment Intent ID: #{charge.payment_intent}" if reservation.nil?
